...
(function (global, factory) {
  ...
  var CompileAnimationQueryMetadata = (function (_super) {
      __extends$1(CompileAnimationQueryMetadata, _super);
      function CompileAnimationQueryMetadata(selector, config, animation) {
          _super.call(this);
          this.selector = selector;
          this.config = config;
          this.animation = animation;
      }
      return CompileAnimationQueryMetadata;
  }(CompileAnimationMetadata));
  ...
  var queryAndRun = _angular_core.__core_private__.queryAndRun;
  ...
  var AnimationQueryAst = (function (_super) {
      __extends$12(AnimationQueryAst, _super);
      function AnimationQueryAst(selector, config, propName, animation) {
          _super.call(this);
          this.selector = selector;
          this.config = config;
  ...
  var CompiledAnimationQuery = (function () {
  ...
  var _ANIMATION_FACTORY_QUERIES_VAR = variable('queries');
  ...
  var _AnimationBuilder = (function () {
      ...
      _AnimationBuilder.prototype.visitAnimationQuery = function (ast, context) {
          var QUERY_FN = importExpr(resolveIdentifier(Identifiers.queryAndRun));
          ...
          var innerAnimationStmt = new ReturnStatement(innerAnimationExpr);
          return QUERY_FN.callFn([
              _ANIMATION_FACTORY_QUERIES_VAR.key(literal(index)),
              fn([
      ...
      _AnimationBuilder.prototype.visitAnimationEntry = function (ast, context) {
          ...
          return fn([
              ...
              new FnParam(_ANIMATION_FACTORY_ELEMENT_VAR.name, DYNAMIC_TYPE),
              new FnParam(_ANIMATION_FACTORY_QUERIES_VAR.name, DYNAMIC_TYPE),
  ...
  var AnimationParser = (function () {
      ...
      AnimationParser.prototype.parseEntry = function (entry) {
          ...
          var stateTransitionAsts = transitions.map(function (transDef) { return _parseAnimationStateTransition(entry, transDef, stateStyles, errors); });
  ...
  function _parseAnimationStateTransition(entryMetadata, transitionStateMetadata, stateStyles, errors) {
      ...
      var animationAst = _parseTransitionAnimation(animation, entryMetadata, 0, styles, stateStyles, errors);
      if (errors.length == 0) {
  ...
  function _parseTransitionAnimation(data, entryMetadata, currentTime, collectedStyles, stateStyles, errors) {
      var ast;
      var playTime = 0;
      var startingTime = currentTime;
      if (data instanceof CompileAnimationQueryMetadata) {
          var innerData = _normalizeAnimationEntry(data.animation);
          var selector = data.selector;
          var normalizedQueryValue = selector.name
              ? selector.name
              : selector.replace(/[^\w-]+/g, '_');
          var queryPropName = entryMetadata.name + "_" + normalizedQueryValue + "_" + QUERY_COUNT++ + "_query";
          ast = new AnimationQueryAst(data.selector, data.config, queryPropName, _parseTransitionAnimation(innerData, entryMetadata, currentTime, collectedStyles, stateStyles, errors));
      }
      else if (data instanceof CompileAnimationWithStepsMetadata) {
          var maxDuration = 0;
          var steps = [];
          var isGroup = data instanceof CompileAnimationGroupMetadata;
          var previousStyles;
          data.steps.forEach(function (step) {
              // these will get picked up by the next step...
              var time = isGroup ? startingTime : currentTime;
              if (step instanceof CompileAnimationStyleMetadata) {
                  step.styles.forEach(function (stylesEntry) {
                      // by this point we know that we only have stringmap values
                      var map = stylesEntry;
                      Object.keys(map).forEach(function (prop) { collectedStyles.insertAtTime(prop, time, map[prop]); });
                  });
                  previousStyles = step.styles;
                  return;
              }
              var innerAst = _parseTransitionAnimation(step, entryMetadata, time, collectedStyles, stateStyles, errors);
      ...
      else if (data instanceof CompileAnimationAnimateMetadata) {
          var timings = _parseTimeExpression(data.timings, errors);
  ...
  var CompileElement = (function (_super) {
      ...
      function CompileElement(parent, view, nodeIndex, renderNode, sourceAst, component, _directives, _resolvedProvidersArray, hasViewContainer, hasEmbeddedView, references) {
          ...
          if (sourceAst instanceof ElementAst) {
              sourceAst.inputs.forEach(function (input) {
                  if (input.isAnimation) {
                      _this.view.animationsSummaryMap[input.name].queries.forEach(function (query) {
                          var queryMetadata = new CompileQueryMetadata({
                              selectors: [
                                  new CompileTokenMetadata({ value: query.selector })
                              ]
                          });
                          _this._addQuery(queryMetadata, null, query.propName);
                      });
                  }
              });
          }
      ...
      CompileElement.prototype._addQuery = function (queryMeta, directiveInstance, propName) {
          if (propName === void 0) { propName = null; }
          propName = propName || "_query_" + queryMeta.selectors[0].name + "_" + this.nodeIndex + "_" + this._queryCount++;
  ...
  var CompileView = (function () {
      function CompileView(component, genConfig, pipeMetas, styles, animations, animationSummary, viewIndex, declarationElement, templateVariableBindings) {
          var _this = this;
          this.component = component;
          ...
          this.animationSummary = animationSummary;
          ...
          animationSummary.entries.forEach(function (entry) {
              _this.animationsSummaryMap[entry.ast.name] = entry;
          });
      }
              ...
              case exports.PropertyBindingType.Animation:
                  ...
                  if (animationSummary && animationSummary.queries) {
                      animationSummary.queries.forEach(function (query) {
                          queryExprs_1.push(THIS_EXPR.prop(query.propName));
                      });
                  }
                  var queryVals = literalArr(queryExprs_1);
                  updateStmts.push(animationTransitionVar_1
                      .set(animationFnExpr.callFn([
                      THIS_EXPR, renderNode, queryVals, oldRenderValue.equals(unitializedValue)
                          .conditional(emptyStateValue, oldRenderValue),
                      renderValue.equals(unitializedValue).conditional(emptyStateValue, renderValue)
                  ]))
                      .toDeclStmt());
                  detachStmts_1.push(animationTransitionVar_1
                      .set(animationFnExpr.callFn([THIS_EXPR, renderNode, queryVals, oldRenderValue, emptyStateValue]))
                      .toDeclStmt());
                  eventListeners.forEach(function (listener) {
  ...
  var ViewBuilderVisitor = (function () {
      ...
      ViewBuilderVisitor.prototype.visitElement = function (ast, parent) {
          ...
          ast.inputs.forEach(function (input) {
              if (input.isAnimation) {
                  var trigger = _this.view.animationSummary.findEntry(input.name);
              }
          });
          return null;
      };
      ViewBuilderVisitor.prototype.visitEmbeddedTemplate = function (ast, parent) {
          ...
          var embeddedView = new CompileView(this.view.component, this.view.genConfig, this.view.pipeMetas, NULL_EXPR, this.view.animations, this.view.animationSummary, this.view.viewIndex + this.nestedViewCount, compileElement, templateVariableBindings);
  ...
  function generateDetectChangesMethod(view) {
      ...
      ListWrapper.addAll(afterContentStmts, view.animationBindingsMethod.finish());
  ...
  var ViewCompiler = (function () {
      ...
      ViewCompiler.prototype.compileComponent = function (component, template, styles, pipes, animationSummary, compiledAnimations) {
          var dependencies = [];
          var view = new CompileView(component, this._genConfig, pipes, styles, compiledAnimations, animationSummary, 0, CompileElement.createNull(), []);
          var statements = [];
  ...
  var AnimationSummary = (function () {
      function AnimationSummary(entries) {
          this.entries = entries;
      }
      AnimationSummary.prototype.findEntry = function (name) {
          var result = this.entries.find(function (entry) {
  ...
  var AnimationEntrySummary = (function () {
  ...
  function generateAnimationSummary(asts) {
  ...
  var OfflineCompiler = (function () {
      ...
      OfflineCompiler.prototype._compileComponent = function (compMeta, directives, pipes, schemas, componentStyles, fileSuffix, targetStatements) {
          ...
          var animationSummary = generateAnimationSummary(parsedAnimations);
          ...
          var viewResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, stylesExpr, pipes, animationSummary, compiledAnimations);
          if (componentStyles) {
  ...
  var CompileMetadataResolver = (function () {
      ...
      CompileMetadataResolver.prototype.getAnimationMetadata = function (value) {
          ...
          else if (value instanceof _angular_core.AnimationQueryMetadata) {
              var steps = value.steps.map(function (step) { return _this.getAnimationMetadata(step); });
              return new CompileAnimationQueryMetadata(value.selector, value.config, steps);
          }
          return null;
      };
  ...
  */
  ...
  var RuntimeCompiler = (function () {
      ...
      RuntimeCompiler.prototype._compileTemplate = function (template) {
          ...
          var compiledAnimations = this._animationCompiler.compile(compMeta.type.name, parsedAnimations);
          var compileResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, variable(stylesCompileResult.componentStylesheet.stylesVar), template.viewPipes, animationSummary, compiledAnimations);
          compileResult.dependencies.forEach(function (dep) {
              var depTemplate;
              if (dep instanceof ViewFactoryDependency) {
                  var vfd = dep;
